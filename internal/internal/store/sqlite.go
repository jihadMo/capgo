...
// ContractStats returns aggregate event metrics for a single contract,
// grouped by event type.
func (s *SQLite) ContractStats(ctx context.Context, contractID string, sc Scope) (ContractStats, error) {
	if err := sc.AllowsContract(contractID); err != nil {
		return ContractStats{}, err
	}
	query := `
		SELECT type, count(*), min(ledger), max(ledger)
		FROM events
		WHERE contract_id = ?
		GROUP BY type
	`
	rows, err := s.db.QueryContext(ctx, query, contractID)
	if err != nil {
		return ContractStats{}, fmt.Errorf("loading contract stats for %s: %w", contractID, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		var typ string
		var cnt, minL, maxL int64
		if err := rows.Scan(&typ, &cnt, &minL, &maxL); err != nil {
			return ContractStats{}, err
		}
		typeCounts[typ] = cnt
		totalEvents += cnt
		if firstLedger == 0 || minL < firstLedger {
			firstLedger = minL
		}
		if maxL > lastLedger {
			lastLedger = maxL
	...
	if totalEvents == 0 {
		return ContractStats{}, ErrNotFound
	}
...
// GetContractSummary returns aggregate metrics for a single contract.
func (s *SQLite) GetContractSummary(ctx context.Context, contractID string, sc Scope) (ContractSummary, error) {
	if err := sc.AllowsContract(contractID); err != nil {
		return ContractSummary{}, err
	}
	query := `
		SELECT contract_id, count(*), min(ledger), max(ledger), max(created_at)
		FROM events
		WHERE contract_id = ?
	...
	err := s.db.QueryRowContext(ctx, query, contractID).Scan(&cs.ContractID, &cs.EventCount, &cs.FirstLedger, &cs.LastLedger, &lastSeen)
	if errors.Is(err, sql.ErrNoRows) {
		return ContractSummary{}, ErrNotFound
	}
	if err != nil {
		return ContractSummary{}, fmt.Errorf("loading contract summary for %s: %w", contractID, err)
	}
