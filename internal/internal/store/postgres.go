...
func (p *Postgres) ContractStats(ctx context.Context, contractID string, sc Scope) (ContractStats, error) {
...
func (p *Postgres) GetContractSummary(ctx context.Context, contractID string, sc Scope) (ContractSummary, error) {
	if err := sc.AllowsContract(contractID); err != nil {
	...
	err := p.withStatementTimeoutTx(ctx, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx, query, contractID).Scan(&s.ContractID, &s.EventCount, &s.FirstLedger, &s.LastLedger, &lastSeen)
	})
	if errors.Is(err, pgx.ErrNoRows) {
