...
// ContractStats holds aggregate metrics and type breakdown for a single contract.
type ContractStats struct {
	ContractID  string           `json:"contract_id"`
	TotalEvents int64            `json:"total_events"`
	FirstLedger int64            `json:"first_ledger"`
	LastLedger  int64            `json:"last_ledger"`
	TypeCounts  map[string]int64 `json:"types"`
}
...
type Store interface {
	...
	ContractStats(ctx context.Context, contractID string, sc Scope) (ContractStats, error)
	// GetContractSummary returns aggregate metrics for a single contract.
	GetContractSummary(ctx context.Context, contractID string, sc Scope) (ContractSummary, error)
