...
func (s *guardedStore) ContractStats(ctx context.Context, contractID string, sc Scope) (ContractStats, error) {
	ctx, cancel := s.wrapContext(ctx, "store.ContractStats")
	defer cancel()
	start := time.Now()
	res, err := s.Store.ContractStats(ctx, contractID, sc)
	s.logSlowQuery("store.ContractStats", start, err)
	return res, err
}

func (s *guardedStore) GetContractSummary(ctx context.Context, contractID string, sc Scope) (ContractSummary, error) {
	ctx, cancel := s.wrapContext(ctx, "store.GetContractSummary")
	defer cancel()
	start := time.Now()
	res, err := s.Store.GetContractSummary(ctx, contractID, sc)
