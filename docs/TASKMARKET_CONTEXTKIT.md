# ContextKit + TaskMarket Integration Guide

Integration guide for publishing ContextKit x402 skill tasks on TaskMarket (Base Mainnet / USDC).
...
npx @lucid-agents/taskmarket task create \
  --title "ContextKit x402 Skill Verification" \
  --reward 5 \
  --currency USDC \
  --network base
```
