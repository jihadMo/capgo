# AgentKit + TaskMarket Integration Guide

Integration guide for publishing AgentKit crypto-wallet agent tasks on TaskMarket (Base Mainnet / USDC).
...
npx @lucid-agents/taskmarket task create \
  --title "AgentKit Wallet Verification Task" \
  --reward 5 \
  --currency USDC \
  --network base
```
