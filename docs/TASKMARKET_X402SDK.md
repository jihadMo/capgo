# x402sdk + TaskMarket Integration Guide

Integration guide for publishing x402 SDK task payments on TaskMarket (Base Mainnet / USDC).
...
npx @lucid-agents/taskmarket task create \
  --title "x402 SDK Base Mainnet Verification" \
  --reward 5 \
  --currency USDC \
  --network base
```
