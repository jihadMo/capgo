# OpenAlice + TaskMarket Integration Guide

Integration guide for publishing OpenAlice trading agent strategy tasks on TaskMarket (Base Mainnet / USDC).
...
npx @lucid-agents/taskmarket task create \
  --title "OpenAlice Trading Strategy Task" \
  --reward 5 \
  --currency USDC \
  --network base
```
