# SpiceAI + TaskMarket Integration Guide

Integration guide for publishing SpiceAI data agent tasks on TaskMarket (Base Mainnet / USDC).
...
npx @lucid-agents/taskmarket task create \
  --title "SpiceAI Data Engine Task" \
  --reward 5 \
  --currency USDC \
  --network base
```
