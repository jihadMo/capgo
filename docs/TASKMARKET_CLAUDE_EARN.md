# ClaudeEarnSelf + TaskMarket Integration Guide

Integration guide for publishing high-priority Claude runtime tasks on TaskMarket (Base Mainnet / USDC).
...
npx @lucid-agents/taskmarket task create \
  --title "Claude Runtime Demand Task" \
  --reward 5 \
  --currency USDC \
  --network base
```
