# Nucleus + TaskMarket Integration Guide

Integration guide for publishing Nucleus open-source workflow agent tasks on TaskMarket (Base Mainnet / USDC).
...
npx @lucid-agents/taskmarket task create \
  --title "Nucleus Agent Workflow Task" \
  --reward 5 \
  --currency USDC \
  --network base
```
